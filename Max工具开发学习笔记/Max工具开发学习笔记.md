...
这种函数第一个参数必须是列表类型，在执行时，会自动遍历列表的每一个项，代替第一个参数进行函数体的计算。

列表有多少个项，这个函数就会执行多少次。就是一个自动遍历的函数，挺方便的。

做完SCT后留言：我超，这个东西挺好用的，但是在做的时候完全忘了它的存在
...
做SKT的时候在这里吃亏了，一开始我觉得MXS的数组和C语言的一样，但其实完全不是。

这样使用数组的话可以快速地查询信息，比如把顶点的索引、或者骨骼ID的索引作为Key，这样去查值是极快的，而且不用受连续自然数这样的限制，可以只在自己需要的索引处赋值。
...
查询次数多了效率是非常低的，比如在工具SCT中，需要多次查询权重、顶点位置等信息，这是非常耗时的。
...
有时候会出现两个函数循环引用，这时不管先定义了谁，先定义的都会报错，因为先定义的引用的函数还没定义。
