...
GAMES202中也有关于SSAO的记录，但是因为笔记目前还没修复，所以不链接到那里。**（//TODO:修复后链接）**
...
> 以下是一些常用的属性标签：
>
> - [Range(min, max)]: 创建一个滑块控件，用于调整属性值在指定范围内的值。
>
> - [Header("text")]: 在属性之前添加一个标题，用于分组和组织属性。
>
> - [Tooltip("text")]: 添加一个工具提示，当鼠标悬停在属性上时会显示该提示。
>
> - [Space(n)]: 在属性之前添加一个空白行，用于在面板中分组和组织属性。
>
> - [TextArea(n, m)]: 创建一个文本框控件，使得可以在面板中编辑多行文本。
>
> - [Toggle("text")]: 创建一个复选框控件，用于启用或禁用属性。
>
> - [ColorUsage]: 创建一个颜色选择器，用于选择颜色。
>
> - [GradientUsage]: 创建一个渐变选择器，用于选择渐变。
>
> - [Enum]: 创建一个下拉列表控件，用于选择指定的枚举值。
>
> - [Multiline(n)]: 创建一个文本框控件，使得可以在面板中编辑多行文本，可以指定行数。
>
> - [Mask]: 创建一个掩码选择器，用于选择位掩码值。
